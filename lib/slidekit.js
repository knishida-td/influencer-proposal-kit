...
async function generate(data, outPath) {
  ...
  // ═══════════════════════════════════════════════
  {
    ...
    if (badges[q.cert1.badgeKey]) s.addImage({ data: badges[q.cert1.badgeKey], x: 7.3, y: top + 0.35, w: 1.5, h: 1.5 });
    addSep(s, 0.5, top + 1.95, 6.5);
    addDefBlock(s, 0.5, top + 2.1, q.cert2.title, q.cert2.body, 6.0);
    if (badges[q.cert2.badgeKey]) s.addImage({ data: badges[q.cert2.badgeKey], x: 7.3, y: top + 2.0, w: 1.5, h: 1.5 });
