...
async function generate(data, outPath) {
  ...
  // ═══════════════════════════════════════════════
  {
    ...
    if (badges[q.cert1.badgeKey]) s.addImage({ data: badges[q.cert1.badgeKey], x: 7.2, y: top + 0.5, w: 1.2, h: 1.2 });
    addSep(s, 0.5, top + 1.5, 9);
    addDefBlock(s, 0.5, top + 1.7, q.cert2.title, q.cert2.body, 6.0);
    if (badges[q.cert2.badgeKey]) s.addImage({ data: badges[q.cert2.badgeKey], x: 7.2, y: top + 1.8, w: 1.2, h: 1.2 });
